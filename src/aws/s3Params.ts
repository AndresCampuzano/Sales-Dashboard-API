import { PutObjectCommand } from '@aws-sdk/client-s3';
import { ObjectId } from 'mongodb';

interface S3Params {
  params: PutObjectCommand;
  id: string;
}

const getContentType = (bf: Buffer) => {
  const types: { [key: string]: string } = {
    jpg: 'image/jpeg',
    png: 'image/png',
    gif: 'image/gif'
  };
  const extension = bf.toString('ascii', 0, 3);
  return types[extension];
};

/**
 * Returns the parameters to upload an image to S3
 */
export const s3Params = (image: string): S3Params => {
  const buffer = Buffer.from(
    image.replace(/^data:image\/\w+;base64,/, ''),
    'base64'
  );
  const id = new ObjectId();
  const params = new PutObjectCommand({
    GrantRead: 'uri=http://acs.amazonaws.com/groups/global/AllUsers', // This is necessary to make the image public
    Bucket: process.env.AWS_S3_BUCKET_NAME as string, // bucket that we made earlier
    Key: id.toString(), // Name of the image
    Body: buffer, // Body which will contain the image in buffer format
    ContentEncoding: 'base64',
    ContentType: getContentType(buffer) // Necessary to define the image content-type to view the photo in the browser with the link
  });
  return {
    params,
    id: id.toString()
  };
};
